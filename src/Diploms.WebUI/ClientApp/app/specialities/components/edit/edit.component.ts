...
export class SpecialitiesEditComponent implements OnInit {
    ...
    ngOnInit() {
        this.service.getSpeciality(this.id).subscribe(result => {
            this.form.setValue(result);
        })
    } 
    
    submit(form: any) {
        this.service.editSpeciality(this.id, form).subscribe(result => this.checkResult(result));
    }
    ...
    remove(){
        this.service.deleteSpeciality(this.id).subscribe(result=> this.checkResult(result));
    }
}
